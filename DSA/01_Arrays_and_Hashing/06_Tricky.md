...
links: ["[[00_Index]]", "[[05_Problems_and_Exercises]]"]
---

# Arrays & Hashing — Tricky & Higher-Order Problems

*← [[05_Problems_and_Exercises|Problems]] · [[../02_Two_Pointers/00_Index|Two Pointers →]]*
...
int maxSumAfterDeletion(std::vector<int>& nums) {
    int n = nums.size();
    std::vector<int> left(n), right(n);

    left[0] = nums[0];
    for (int i = 1; i < n; i++)
        left[i] = std::max(nums[i], left[i-1] + nums[i]);

    right[n-1] = nums[n-1];
    for (int i = n-2; i >= 0; i--)
        right[i] = std::max(nums[i], right[i+1] + nums[i]);

    int ans = *std::max_element(left.begin(), left.end()); // delete nothing
    for (int i = 1; i < n-1; i++)
        ans = std::max(ans, left[i-1] + right[i+1]);
...
int minOperations(std::vector<int>& nums) {
    ...
    for (int i = 0; i < m; i++) {
        ...
        int end = nums[i] + n - 1;
        int j = (int)(std::upper_bound(nums.begin(), nums.end(), end) - nums.begin());
        best = std::max(best, j - i);  // j-i values we can keep
    }
    return n - best;
}
...
```cpp
long long mergeCount(std::vector<long long>& prefix, int left, int right,
                     int lower, int upper) {
    ...
    std::inplace_merge(prefix.begin() + left,
                       prefix.begin() + mid,
                       prefix.begin() + right);
    return count;
}
...
std::string minWindow(std::string s, std::string t) {
    std::unordered_map<char,int> need, have_map;
    for (char c : t) need[c]++;
...
Always use custom hash (shown in [[03_Hashing|Hashing notes]]) when using `unordered_map` or `unordered_set` in competitive settings.
